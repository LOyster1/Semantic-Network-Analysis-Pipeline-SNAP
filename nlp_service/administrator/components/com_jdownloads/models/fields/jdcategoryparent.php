 ...
 * @since		1.6
 */
class JFormFieldjdCategoryParent extends JFormFieldList
{
	/**
	 * The form field type.
	 *
	...
	protected function getOptions()
	{
        ...
        for ($i = 0, $n = count($options); $i < $n; $i++)
        {
            ...
            if ($options[$i]->level == 0) {
                $options[$i]->text = JText::_('COM_JDOWNLOADS_EDIT_CAT_NO_PARENT_OPTION');
            }
        ...
        if (empty($id)) {
            ...
            foreach ($options as $i => $option)
            {
                ...
                if (!$user->authorise('core.create', 'jdownloads.category.'.$option->value)) {
                    unset($options[$i]);
                }
            }
        }
            ...
            {
                // Unset the option if the user isn't authorised for it.
                if (!$user->authorise('core.edit', 'jdownloads.category.'.$option->value)) {
                    // As a backup, check core.edit.own
                    if (!$user->authorise('core.edit.own', 'jdownloads.category.'.$option->value)) {
